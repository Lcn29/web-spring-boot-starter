...
public class JacksonObjectMapperBuilderCustomizer {
    ...
    public <T> JacksonObjectMapperBuilderCustomizer addJsonSerializer(Class<T> classType, JsonDeserializer<T> jsonDeserializer) {
        customerJsonDeserializerMap.put(classType.getName(),
            new CustomerJsonDeserializer<>(classType, jsonDeserializer));
        return this;
    }
    ...
    public Jackson2ObjectMapperBuilderCustomizer generateJackson2ObjectMapperBuilderCustomizer() {
        return jacksonObjectMapperBuilder -> {
            ...
            // 自定义的序列化器
            customerJsonSerializerMap.values().forEach(
                item -> jacksonObjectMapperBuilder.serializerByType(item.classType, item.jsonSerializer));

            // 自定义的反序列化器
            customerJsonDeserializerMap.values().forEach(
                item -> jacksonObjectMapperBuilder.deserializerByType(item.classType, item.jsonDeserializer));
    ...
    private void localDateTimeHandler(Jackson2ObjectMapperBuilder jacksonObjectMapperBuilder) {
        ...
        DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern(StringConstants.DATE_TIME_FORMAT);
